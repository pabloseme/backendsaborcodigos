...
const pedidoPost=async(req, res)=>{
    ...
    const {fecha,menu,cant,preciounit}= req.body

    const pedid=new Pedido({fecha,menu,cant,preciounit})
...
const pedidoPut= async(req, res)=>{
    ...
    const pedidonew= await Pedido.findByIdAndUpdate(id,resto,{new:true})
    res.json({
